...
# %% 결과별 정리
# 01_cancer
rslt_01 = 'Stomach cancer'
rslt_01_cd = ['G100','G101','G104']
...
rslt_02 = 'Esophageal cancer'
rslt_02_cd = ['G100','G101','G104']
for i in range(len(rslt_02_cd)):
    data['rslt_02_{}'.format(i)] = (
                     (data.loc[:,'BP1A155_RSLT_CD01':'BP1A155_RSLT_CD07'] == rslt_02_cd[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_02_cd)):
    data.loc[data['rslt_02_{}'.format(i)] == 1,rslt_02] = '01_'+rslt_02
data[rslt_02].fillna('00_Absent',inplace=True)

rslt_03 = 'Duodenal cancer'
rslt_03_cd = ['G100','G101','G104']
for i in range(len(rslt_03_cd)):
    data['rslt_03_{}'.format(i)] = (
                     (data.loc[:,'BP1A154_RSLT_CD01':'BP1A154_RSLT_CD07'] == rslt_03_cd[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_03_cd)):
    data.loc[data['rslt_03_{}'.format(i)] == 1,rslt_03] = '01_'+rslt_03
data[rslt_03].fillna('00_Absent',inplace=True)

rslt_04 = 'Adenoma, stomach'
rslt_04_cd = ['G051','G052','G080','G083','G301','G303','G304']
for i in range(len(rslt_04_cd)):
    data['rslt_04_{}'.format(i)] = (
                     (data.loc[:,'BP1A151_RSLT_CD01':'BP1A151_RSLT_CD07'] == rslt_04_cd[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_04_cd)):
    data.loc[data['rslt_04_{}'.format(i)] == 1,rslt_04] = '01_'+rslt_04
data[rslt_04].fillna('00_Absent',inplace=True)

rslt_05 = 'Adenoma, Esophagus'
rslt_05_cd = ['G051','G080','G083','G301','G303','G304']
for i in range(len(rslt_05_cd)):
    data['rslt_05_{}'.format(i)] = (
                     (data.loc[:,'BP1A155_RSLT_CD01':'BP1A155_RSLT_CD07'] == rslt_05_cd[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_05_cd)):
    data.loc[data['rslt_05_{}'.format(i)] == 1,rslt_05] = '01_'+rslt_05
data[rslt_05].fillna('00_Absent',inplace=True)

rslt_06 = 'Adenoma, duodenal'
# duodenal biopsy
rslt_06_cd1 = ['D406','G051','G080','G083','G303']
for i in range(len(rslt_06_cd1)):
    data['rslt_06_1_{}'.format(i)] = (
                     (data.loc[:,'BP1A154_RSLT_CD01':'BP1A154_RSLT_CD07'] == rslt_06_cd1[i])
                    ).any(axis=1).astype(int)
# stomach biopsy
rslt_06_cd2 = ['D406']
for i in range(len(rslt_06_cd2)):
    data['rslt_06_2_{}'.format(i)] = (
                     (data.loc[:,'BP1A151_RSLT_CD01':'BP1A151_RSLT_CD07'] == rslt_06_cd2[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_06_cd1)):
    data.loc[data['rslt_06_1_{}'.format(i)] == 1,rslt_06] = '01_'+rslt_06
for i in range(len(rslt_06_cd2)):
    data.loc[data['rslt_06_2_{}'.format(i)] == 1,rslt_06] = '01_'+rslt_06
data[rslt_06].fillna('00_Absent',inplace=True)

rslt_07 = 'Low Grade MALTOMA'
# stomach BP
rslt_07_cd1 = ['G102','G103']
for i in range(len(rslt_07_cd1)):
    data['rslt_07_1_{}'.format(i)] = (
                     (data.loc[:,'BP1A151_RSLT_CD01':'BP1A151_RSLT_CD07'] == rslt_07_cd1[i])
                    ).any(axis=1).astype(int)
# Esophagus BP
rslt_07_cd2 = ['G102','G103']
for i in range(len(rslt_07_cd2)):
    data['rslt_07_2_{}'.format(i)] = (
                     (data.loc[:,'BP1A155_RSLT_CD01':'BP1A155_RSLT_CD07'] == rslt_07_cd2[i])
                    ).any(axis=1).astype(int)
# Duodenal BP
rslt_07_cd3 = ['G102','G103']
for i in range(len(rslt_07_cd3)):
    data['rslt_07_3_{}'.format(i)] = (
                     (data.loc[:,'BP1A154_RSLT_CD01':'BP1A154_RSLT_CD07'] == rslt_07_cd3[i])
                    ).any(axis=1).astype(int)
for i in range(len(rslt_07_cd1)):
    data.loc[data['rslt_07_1_{}'.format(i)] == 1,rslt_07] = '01_'+rslt_07
for i in range(len(rslt_07_cd2)):
    data.loc[data['rslt_07_2_{}'.format(i)] == 1,rslt_07] = '01_'+rslt_07
for i in range(len(rslt_07_cd3)):
    data.loc[data['rslt_07_3_{}'.format(i)] == 1,rslt_07] = '01_'+rslt_07
data[rslt_07].fillna('00_Absent',inplace=True)

data

# #%%
# data.to_excel('{}/EGD_SAMPLE.xlsx'.format(workdir[:-5]),index=False)
...
for i in range(1,8):
    globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)] = data_org.pivot_table(
    ...
    globals()['EGD_rslt_0{}_per_{}'.format(i,gend_cd)] = round(globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)].div(globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)].iloc[-1], axis=1).astype(float)*100,1)

    # globals()['EGD_rslt_0{}_per_m'.format(i)]

    globals()['EGD_rslt_0{}_agegrp_{}'.format(i,gend_cd)] = pd.DataFrame()

    for j in range(len(globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)].columns)):
        if j == 0:
            globals()['EGD_rslt_0{}_agegrp_{}'.format(i,gend_cd)] = pd.concat(
                                    [
                                    globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)].iloc[:,j]
                                    ,globals()['EGD_rslt_0{}_per_{}'.format(i,gend_cd)].iloc[:,j]
                                    ]
                                ,axis=1
            )
        else:
            globals()['EGD_rslt_0{}_agegrp_{}'.format(i,gend_cd)] = pd.concat(
                                    [
                                    globals()['EGD_rslt_0{}_agegrp_{}'.format(i,gend_cd)]
                                    ,globals()['EGD_rslt_0{}_cnt_{}'.format(i,gend_cd)].iloc[:,j]
                                    ,globals()['EGD_rslt_0{}_per_{}'.format(i,gend_cd)].iloc[:,j]
                                    ]
                                ,axis=1
...
# for i in range(1,8):
#     print(globals()['EGD_rslt_0{}_agegrp_{}'.format(i,gend_cd)])

# data merge
globals()['EGD_rslt_{}'.format(gend_cd)] = pd.concat(
[
    globals()['EGD_rslt_01_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_02_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_03_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_04_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_05_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_06_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_07_agegrp_{}'.format(gend_cd)].iloc[1,:]
   ,globals()['EGD_rslt_07_agegrp_{}'.format(gend_cd)].iloc[-1,:] # 전체인원
]
,axis=1
         ).T

globals()['EGD_rslt_{}'.format(gend_cd)].columns = pd.MultiIndex.from_tuples(
        (
         ('29세 이하', 'N')
        ,('29세 이하', '%')
        ,('30~39세', 'N')
...
globals()['EGD_rslt_{}'.format(gend_cd)]
...
with pd.ExcelWriter('{}/FACTSHEET_2020_TABLE.xlsx'.format(workdir[:-5]), mode='a',engine='openpyxl') as writer:
    ...
    EGD_rslt_m.to_excel(writer,sheet_name="03_12_EGDBP_m")
    EGD_rslt_f.to_excel(writer,sheet_name="03_12_EGDBP_f")
...
# #%%            
# for i in range(1,8):
#     globals()['EGD_rslt_0{}_cnt_t'.format(i)] = data.pivot_table(
#                              index=[globals()['rslt_0{}'.format(i)],'GENDER']
#                             ,columns=['AGEGRP']
#                             ,values=['ID']
#                             ,aggfunc='count'
#                             ,margins=True
...
#     # EGD_rslt_01_cnt_t
#     # each column total value percentile
#     globals()['EGD_rslt_0{}_per_t'.format(i)] = round(globals()['EGD_rslt_0{}_cnt_t'.format(i)].div(globals()['EGD_rslt_0{}_cnt_t'.format(i)].iloc[-1], axis=1).astype(float)*100,1)

#     # globals()['EGD_rslt_0{}_per_t'.format(i)]

#     globals()['EGD_rslt_0{}_agegrp_t'.format(i)] = pd.DataFrame()

#     for j in range(len(globals()['EGD_rslt_0{}_cnt_t'.format(i)].columns)):
#         if j == 0:
#             globals()['EGD_rslt_0{}_agegrp_t'.format(i)] = pd.concat(
#                                     [
#                                     globals()['EGD_rslt_0{}_cnt_t'.format(i)].iloc[:,j]
#                                     ,globals()['EGD_rslt_0{}_per_t'.format(i)].iloc[:,j]
#                                     ]
#                                 ,axis=1
#             )
#         else:
#             globals()['EGD_rslt_0{}_agegrp_t'.format(i)] = pd.concat(
#                                     [
#                                     globals()['EGD_rslt_0{}_agegrp_t'.format(i)]
#                                     ,globals()['EGD_rslt_0{}_cnt_t'.format(i)].iloc[:,j]
#                                     ,globals()['EGD_rslt_0{}_per_t'.format(i)].iloc[:,j]
#                                     ]
#                                 ,axis=1
...
# #%%
# for i in range(1,8):
#     print(globals()['EGD_rslt_0{}_agegrp_t'.format(i)])
